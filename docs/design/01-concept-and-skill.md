...
├── hooks/guard.py               # what the hooks run: imports a guard from src/ with any Python 3.11+
├── src/hungry_crab/
│   ├── cli.py
...
| Which nutrients become pull requests | `serve --as pr-branch` publishes REIMPLEMENT nutrients from the clean-room receipt and COPY / COPY_FILE nutrients from a materialization receipt — which maw path came from which prey path, verbatim or adapted — verified against the prey at that commit; the receipt joins `.crab/attributions.json`, the notice file is rendered from all receipts, and files, receipts and notice travel in one pull request | a receipt is the only producer-side statement of which files belong to a nutrient; a dirty working tree is never inferred as the payload; and a notice must name what was actually carried in, which neither a ledger status (an issue takes nothing) nor the prey that first *proposed* a nutrient can say |
...
| Isolating the clean-room implementer | a `PreToolUse` hook (`crab-cleanroom-guard`) keyed on the agent, not a frontmatter deny rule; both hooks run `hooks/guard.py` from the plugin root with the first Python the shell finds, and the launcher imports the guard from the plugin's own `src/` | plugin subagents cannot carry path-scoped permissions in their frontmatter, so the hook is the only mechanical boundary available; wired to a console script it was only as present as that script on PATH — absent on a plugin-only install, and present but refused (exit 126, read as "proceed") where an application-control policy rejects unsigned launchers. Exit 2 is reserved for a guard's refusal: a launcher that cannot run exits 1 and says so, because a hook that blocked every call over its own environment would be switched off, not fixed |

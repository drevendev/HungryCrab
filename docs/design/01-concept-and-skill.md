...
| Licenses the engine cannot reduce to one identifier | four named resolutions instead of one `NOASSERTION`: `dual` (alternatives, `OR`), `split` (different parts, `AND`, most restrictive wins), `per-path` (nothing at the root, packages license themselves) and `unreadable` (`HUMAN`) | the four situations need four different actions, and the single verdict hid two outright bugs: `apache-2.0.LICENSE` was invisible and `LICENSE.md` was read as a license named `md` |

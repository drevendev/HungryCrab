...
- `src/hungry_crab/cli.py`: argparse entry point (`crab sniff | catch | digest | compare | menu |
  serve | ledger | tune | init | update | cache | guard`).
- `src/hungry_crab/guard.py`: the answer behind `crab guard`, and behind the `PreToolUse` hook
  in `hooks/hooks.json`: would this shell command execute something out of the prey cache?

...
def _verbs(command: str) -> list[str]:
    """The leading word of every pipeline segment, as a bare program name."""
    found: list[str] = []
    for segment in _SEPARATORS.split(command):
        ...
        for token in tokens:
            if _ASSIGNMENT.match(token):
                continue  # FOO=bar python x.py
            name = Path(token.strip("'\"")).name.lower()
            found.append(name.removesuffix(".exe"))
            break
    return found
